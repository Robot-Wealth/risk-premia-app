...
ew_norebal <- monthlyprices %>%
  ...
  mutate(commission = case_when(trades * perShareComm > minCommPerOrder ~ trades * perShareComm, trades == 0 ~ 0, TRUE ~ minCommPerOrder))
...
# Equal weight, rebalanced every month
...
volsize_daily <- prices %>%
  group_by(ticker) %>%
  arrange(date) %>%
  mutate(returns = (closeadjusted / dplyr::lag(closeadjusted)) - 1) %>%
  mutate(vol = slider::slide_dbl(.x = returns, .f = sd, .before = vol_lookback, .complete = TRUE) * sqrt(252)) %>%
  mutate(volsize = lag(vol_target / vol))

monthendsize <- volsize_daily %>%
  inner_join(monthlyprices, by = 'date') %>%
  group_by(date) %>%
  summarise(totalsize = sum(volsize)) %>%
  mutate(adjfactor = case_when(totalsize > 1 ~ 1/totalsize, TRUE ~ 1))

volsize <- volsize_daily %>%
  inner_join(monthendsize, by = 'date') %>%
  mutate(size = volsize * adjfactor) %>%
  select(ticker, date, vol, volsize, size)
```
